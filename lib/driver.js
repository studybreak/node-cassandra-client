...
var DEFAULT_STEP_TIMEOUTS = module.exports.DEFAULT_STEP_TIMEOUTS = {
  'login': 1000,
  'learn': 2000,
  'use': 1000
};
  ...
  for (var i = 0; i < row.columns.length; i++) {
    if (row.columns[i].value && row.columns[i].name != 'KEY') {
      ...
      this.cols[count] = {
        name: decodedName,
        value: decodedValue,
        timestamp: new Date(row.columns[i].timestamp / 1000)
      };
...
Connection.prototype.execute = function(query, args, callback) {
  var cql,
      metadata = {'connectionInfo': this.connectionInfo, 'timingInfo': {}},
  ...
  self.client.execute_cql_query(cql, ttypes.Compression.NONE, function(err, res) {
    ...
    metadata.timingInfo.queryLatency = diff;
...
PooledConnection.prototype.execute = function(query, args, executeCallback) {
  ...
  var callback = function(err, results, metadata) {
    self.running--;
    if (metadata) metadata.timingInfo.poolLatency = (new Date().getTime() - start);
